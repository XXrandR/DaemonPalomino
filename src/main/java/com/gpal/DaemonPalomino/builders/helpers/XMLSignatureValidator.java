        ...
        try {
            ...
            if (isValid) {
                ...
                for (Object ref : signature.getSignedInfo().getReferences()) {
                    ...
                    log.debug("Reference validation status: " + refValid);
                }

                // Check core validation status
                boolean coreValidation = signature.getSignatureValue().validate(validateContext);
                log.debug("Core validation status: " + coreValidation);
                ...
                for (int i = 0; i < references.size(); i++) {
                    ...
                    log.debug("Reference " + i + " validation status: " + refValid);
                    if (refValid) {
                        log.debug("Reference " + i + " digest: "
                                + Base64.getEncoder().encodeToString(ref.getDigestValue()));
                        log.debug("Reference " + i + " calculated digest: "
                ...
                for (int i = 0; i < references.size(); i++) {
                    ...
                    for (int j = 0; j < transforms.size(); j++) {
                        ...
                        log.debug("Reference " + i + ", Transform " + j + ": " + transform.getAlgorithm());
                    }
                }
